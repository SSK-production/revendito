...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const uploadDir = "public/offer/commercial";
    ...
    const { error } = commercialOfferSchema.validate(
      { ...fields, photos },
      { abortEarly: false }
    );
    ...
    const newCommercialOffer: CommercialOffer =
    ...
    const uniqueDir = path.join(
      ...
      "offer",
      "commercial",
      newCommercialOffer.id.toString()
    );
    ...
    photos.forEach((photo) => {
      ...
      finalPhotoPaths.push(`/offer/commercial/${newCommercialOffer.id}${photo}`);

    });

    // Mettez à jour l'offre avec les chemins des photos
    await prisma.commercialOffer.update({
      where: { id: newCommercialOffer.id },
      data: {
