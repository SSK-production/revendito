...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const uploadDir = "public/offer/property";
    ...
    const newRealEstateOffer: RealEstateOffer =
    ...
    const uniqueDir = path.join(
      ...
      "offer",
      "property",
      newRealEstateOffer.id.toString()
    );
    ...
    photos.forEach((photo) => {
      ...
      // Ajoutez le chemin relatif pour la base de données
      finalPhotoPaths.push(`/offer/property/${newRealEstateOffer.id}${photo}`);
    });

    // Mettez à jour l'offre avec les chemins des photos
     await prisma.realEstateOffer.update({
      where: { id: newRealEstateOffer.id },
      data: {
