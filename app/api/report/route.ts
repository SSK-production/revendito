...
export async function GET(req: NextRequest) {
    try {
      ...
      const response = NextResponse.json({ id, entity, accessToken }, { status: 201 });
      ...
      if (error instanceof Error) {
        return NextResponse.json({ error: 'Erreur lors de la création du report: ' + error.message }, { status: 500 });
      }
      return NextResponse.json({ error: 'Erreur inconnue lors de la création du report' }, { status: 500 });
    }
  }
...
export async function POST(req: NextRequest) {
    try {
        ...
        const { reason, vehicleOfferId, realEstateOfferId, commercialOfferId } = await req.json();

        //vérification des données
        const { error } = reportSchema.validate({ reason, vehicleOfferId, realEstateOfferId, commercialOfferId }, { abortEarly: false });
        if (error) {
