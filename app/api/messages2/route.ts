...
export async function POST(req: NextRequest) {
    try {
        ...
        if (error instanceof Error) {
            return NextResponse.json({ error: 'Erreur lors de la création du message: ' + error.message }, { status: 500 });
        }
        return NextResponse.json({ error: 'Erreur inconnue lors de la création du message' }, { status: 500 });
    }
}